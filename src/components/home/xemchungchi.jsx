...
export default function Index({ masv }) {
  ...
  const [datasvcc, setDatasvcc] = useState([])
  useEffect(() => {
    console.log("gọi lại api")
    const callApi = async () => {

      await fetch(`https://qlvbcc.hasura.app/api/rest/get_minhchung/${masv}/chungchi`, {
        method: 'GET',
        headers: {
  ...
  useEffect(() => {
    fetch(`https://qlvbcc.hasura.app/api/rest/get_chungchi/${masv}`, {
      headers: {
        'Content-Type': 'application/json',
        'x-hasura-admin-secret': 'SeALKpEjjdBd2xlGyBXtGPjU9C46BocE6P3DERIgB8sJhPGvUH57qvh7QnMW4e9c'
      }
      ...
      .then(response => response.json())
      .then(datasvcc => {
        setDatasvcc(datasvcc.f_get_chungchi
        )
      ...
      {datasv && datasv.map((sinhvien, index) => (
        <div key={index} >
          ...
          <div className="flex flex-col justify-center items-center mt-[70px]">
            <h2 className="font-bold text-[30px] ">CHỨNG CHỈ</h2>
          </div>
          ...
          <table className="min-w-full border-collapse border border-gray-300 mt-[20px]">
            <thead>
              <tr>
                <th className="px-4 py-2 bg-gray-100 border-b text-center">STT</th>
                <th className="px-4 py-2 bg-gray-100 border-b text-center">Mã chứng chỉ</th>
                <th className="px-4 py-2 bg-gray-100 border-b text-center">Tên chứng chỉ</th>
                <th className="px-4 py-2 bg-gray-100 border-b text-center">Đợt xét</th>
                <th className="px-4 py-2 bg-gray-100 border-b text-center">Ngày cấp</th>

              </tr>
            </thead>
            <tbody>
              {datasvcc && datasvcc.map((classs, index) => (
                <tr key={index}>
                  <td className="px-4 py-2 border-b text-center">{index+1}</td>
                  <td className="px-4 py-2 border-b text-center">{classs.machungchi}</td>
                  <td className="px-4 py-2 border-b text-center">{classs.ten}</td>
                  <td className="px-4 py-2 border-b text-center">{classs.dotxet}</td>
                  
                  {classs.ngaycap ? <td className="px-4 py-2 border-b text-center">{classs.ngaycap.split('T')[0].split('-').reverse().join('-')}</td> : <td></td>}
                </tr>
              ))}
            </tbody>
          </table>
